	...
	<body ondblclick="fullScreen()">
		...
		<script type="text/javascript">
			...
			function openChannel() {
				...
				//socket = new WebSocket("ws://10.0.7.181:8080/WebRTC/acgist.video/${requestScope.uid}");
				socket = new WebSocket("ws://192.168.1.105:8080/video/acgist.video/${requestScope.uid}");
			...
			function noticeMsg() {
				if (!initiator) {
					//setNotice("让别人加入（注意事项查看源码）: http://10.0.7.181:8080/WebRTC/msg?oid=${requestScope.uid}");
					setNotice("让别人加入（注意事项查看源码）: ws://192.168.1.105:8080/video/msg?oid=${requestScope.uid}");
				} else {
					setNotice("初始化...");
				}
			}
			...
			function createPeerConnection() {
				...
				pc.onremovestream = onRemoteStreamRemoved;1
			}
